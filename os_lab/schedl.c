...
w[1]=0;
printf("\nwaiting time of process %d :0",a[1]);
for(i=2;i<=n;i++)
{
w[i]=b[i-1]+w[i-1];
printf("\nWaiting time of process %d: %d",a[i],w[i]);
avg+=w[i];
}
for(i=1; i<=n; i++)
{
...
e=0;
 ...
 gantt[f][1]=time; 
 printf ("%8s%8s%8s%8s%8s%8s\n\n","Process","Arrival","Burst","Priority","Waiting","Turnarnd"); 
 for (i=0;i<e;i++) 
...
scanf("%d",&n);
n1=n;
printf("\nEnter burst time for the folllowing job:\n");
for(i=1;i<=n;i++)
{
printf("\nProcess %d: ",i);
scanf("%d",&b[i]); z[i]=b[i];
